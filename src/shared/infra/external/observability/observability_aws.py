...
class ObservabilityAWS(IObservability):
    ...
    def presenter_decorators(self, presenter) -> None:
        ...
        def presenter_wrapper(event):
            ...
            self.metrics.add_metric(name="ProcessingTime", unit="Milliseconds",
                                    value=processing_time_ms)  # End - ProcessingTime metrics
    ...
    def handler_decorators(self, handler) -> None:
        ...
        @self.metrics.log_metrics(capture_cold_start_metric=True,
                                  default_dimensions={"environment": os.getenv("STAGE", "dev"), "mss": self.mss_name,
                                                      "module": self.module_name})  # ColdStart metrics and adding dimensions
